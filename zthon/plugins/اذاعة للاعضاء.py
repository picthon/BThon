    ...
    async for usr in event.client.iter_participants(event.chat_id):
        ...
        if zilzal.text:
            ...
            try:
                ...
                return
    ZELZAL_BEST_DEV = f"[ᯓ 𝗕𝗧𝗵𝗼𝗻 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اذاعـة خـاص 🚹](t.me/BThon) .\n\n**⎉╎تمت الاذاعـه لـ اعضـاء الكـروب .. بنجـاح  ✅**\n**⎉╎عـدد {success} عضـو**"
    ...
    for user in users:
        ...
        if zilzal.text:
            ...
            try:
                ...
                zzz.edit(f"خطأ في إرسال الرسالة إلى {user_entity.id}: {str(e)}")
    ZELZAL_BEST_DEV = f"[ᯓ 𝗕𝗧𝗵𝗼𝗻 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اذاعـة زاجـل 🕊](t.me/BThon) .\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n**⎉╎تمت الاذاعـه .. بنجـاح  ✅**\n**⎉╎عـدد {success} أشخـاص**"
